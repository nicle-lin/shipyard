...
remote-build:
	...
	@cd controller && docker run --rm -w /go/src/github.com/dockerclubgroup/shipyard --entrypoint /bin/bash shipyard-build -c "make build 1>&2 && cd controller && tar -czf - controller" | tar zxf -
